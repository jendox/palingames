...
def _send_guest_order_download_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
...
def _send_custom_game_download_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
...
def _send_order_reward_user_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
...
def _send_review_reward_user_notification(
    *,
    ...
    send_review_reward_user_email(review=review, promo_code=promo_code, notification_outbox=outbox)
    Review.objects.filter(pk=review.pk, reward_email_sent_at__isnull=True).update(
        reward_email_sent_at=timezone.now(),
    )


def _send_review_submitted_admin_email_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
    review = outbox.target or Review.objects.select_related("product", "user").get(pk=payload["review_id"])
    send_review_submitted_admin_email(review=review, notification_outbox=outbox)


def _send_custom_game_request_customer_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
    custom_game_request = outbox.target or CustomGameRequest.objects.get(pk=payload["custom_game_request_id"])
    send_custom_game_request_customer_email(custom_game_request=custom_game_request, notification_outbox=outbox)


def _send_custom_game_request_admin_email_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
    custom_game_request = outbox.target or CustomGameRequest.objects.get(pk=payload["custom_game_request_id"])
    send_custom_game_request_admin_email(custom_game_request=custom_game_request, notification_outbox=outbox)


def _send_custom_game_request_admin_telegram_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
...
def _send_invoice_created_user_notification(
    *,
    ...
    send_invoice_created_user_email(invoice=invoice, notification_outbox=outbox)


def _send_auth_account_email_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
...
def _send_payments_monthly_report_admin_telegram_notification(
    *,
    outbox: NotificationOutbox,
    payload: NotificationPayload,
) -> None:
