...
def _get_payment_webhook_failure_cache_key(*, provider: str, reason: str) -> str:
...
def record_payment_webhook_failure_incident(*, provider: str, reason: str) -> bool:
    ...
    cache_key = _get_payment_webhook_failure_cache_key(provider=provider, reason=reason)
    ...
    return send_incident_alert(
        key=f"payments.webhook.failures:{provider}:{reason}",
