(function () {
  const configEl = document.getElementById("product-file-direct-upload-config");
  if (!configEl) return;
  ...
  const statusEl = document.getElementById("product-file-direct-upload-status");
  const form = document.getElementById("productfile_form");
  ...
  function getProductId() {
    if (!productInput || !productInput.value) return null;
    const parsed = Number.parseInt(productInput.value, 10);
  ...
  async function handleFile(file) {
    ...
    if (!productId) {
      setStatus("Сначала выберите продукт.", true);
      uploadInput.value = "";
      return;
    }
    ...
    try {
      ...
      if (config.productFileId) {
        finalizeBody.product_file_id = config.productFileId;
      }
